<?php
/**
 * Funkce vytváří novou stránku pro nově vytvořenou diskuzi (.php soubor)
 *
 * @param $namePage
 * Nazev (id) nove generovane stranky diskuze
 * @param $autor
 * Jmeno autora generovane diskuze
 *
 * Vytvari novou stranku a txt soubor, fungujici jako databaze odpovedi, pro nove vytvorenou diskuzi
 */
function createPage($namePage, $autor){
    $html = '<?php
...
$answers = createDbAnswer("'.$namePage.'");
...
if(isset($_POST["send"])){
    ...
    $emptyInput = addAnswer("'.$namePage.'", $answer,$loggedUser->username);
...
<main class="container"> 
    ...
    <div class="main">
        <?php
        echoDiscussion($diskuze,"'.$namePage.'");
        ?>
        ...
        <h3>
            ...
            changePage($page, $numOfAnswers,"'.$namePage.'" );
            }
            ?>
        </h3>
...
<script src="../js/validAnswer.js"></script>
    ...
    file_get_contents("databaze/$namePage.php");
    file_put_contents("databaze/$namePage.php", $html);
    fopen("databaze/$namePage.txt", "w");
}
